...
class WyseFlowGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("WyseFlow Receiver")
    ...
    def run_receiver(self):
        def callback(ch, method, properties, body):
            try:
                ...
                # Update GUI
                self.update_latest(f"{name} (ID: {student_id}) - {', '.join(programs)} | CPs: {credits}")
    ...
    def show_students(self):
        ...
        popup.title("All WyseFlow Students")
        popup.geometry("500x300")

        if os.path.exists("wyseflow_data.json"):
            with open("wyseflow_data.json", "r") as f:
